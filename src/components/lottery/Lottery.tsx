...
import { compareNumbers, randomNumbers, toggleSelection } from '../../utils'
import { Field, Spinner, Typography } from '../index.ts'
import { Button } from '../button/button.tsx'
import { postRequest } from '../../api/handleSubmit.ts'
import s from './lottery.module.css'

export const Lottery = () => {
    const [selectedFirstField, setSelectedFirstField] = useState<number[]>([])
    const [selectedSecondField, setSelectedSecondField] = useState<number[]>([])
    const [result, setResult] = useState<string | null>(null)
    const [isPending, setIsPending] = useState(false)
    const handleSelectFirstField = (number: number) => setSelectedFirstField(prev => toggleSelection(prev, number))
    const handleSelectSecondField = (number: number) => setSelectedSecondField(prev => toggleSelection(prev, number))
    ...
    const handleCheckResult = async () => {
        ...
        setIsPending(false)
        if (result) {
            setResult('Ого, Вы выиграли! Поздравляем!')
        } else {
            setResult('Упс, Вы проиграли!')
        }
    }
    ...
    const handleGetRandom = () => {
        ...
        setSelectedFirstField(randomFirstField)
        setSelectedSecondField(randomSecondField)
    }
    ...
    return (
        <div className={s.lottery} key={result}>
            ...
            <Field
                ...
                maxSelect={1}
                onSelect={handleSelectSecondField}
            />
            <Button onClick={handleCheckResult}
                    className={s.lotteryButton}
                    disabled={selectedFirstField.length !== 8 || selectedSecondField.length !== 1 || isPending}>
                {isPending && <Spinner /> || 'Показать результат'}
            </Button>
            {result && <p>{result}</p>}
        </div>
    )
